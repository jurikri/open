...
sham0 = nanex(biRNN_long_subset[shamGroup,0])
sham1 = nanex(biRNN_long_subset[shamGroup,1])
sham2 = nanex(biRNN_long_subset[shamGroup,2])
       
psl0 = nanex(biRNN_long_subset[pslGroup,0])
psl1 = nanex(biRNN_long_subset[pslGroup,1])
psl2 = nanex(biRNN_long_subset[pslGroup,2])

adenosine0 = nanex(biRNN_long_subset[adenosineGroup,0])
adenosine1 = nanex(biRNN_long_subset[adenosineGroup,1])
adenosine2 = nanex(biRNN_long_subset[adenosineGroup,2])
...
# I.T. clonidine
tmp = biRNN_long_subset[itClonidineGroup,1:3]
where_132 = np.where(np.array(itClonidineGroup)==132)[0][0]
ex_saline = tmp[where_132,1]
tmp[where_132,1] = np.nan
itClonidineGroup0 = nanex(biRNN_long_subset[itClonidineGroup,0])
itClonidineGroup1 = nanex(tmp[:,0])
itClonidineGroup2 = nanex(tmp[:,1])

# I.T. saline
itSalineGroup0 = nanex(biRNN_long_subset[itSalineGroup,0])
itSalineGroup1 = nanex(biRNN_long_subset[itSalineGroup,1])
tmp = nanex(biRNN_long_subset[itSalineGroup,2])
itSalineGroup2 = np.append(tmp, ex_saline)

# psl 통합
Aprism_biRNN2_psl = pd.DataFrame(sham0)
Aprism_biRNN2_psl = pd.concat([Aprism_biRNN2_psl , pd.DataFrame(sham1), pd.DataFrame(sham2) \
                              , pd.DataFrame(psl0) , pd.DataFrame(psl1), pd.DataFrame(psl2) \
                              , pd.DataFrame(adenosine0) , pd.DataFrame(adenosine1), pd.DataFrame(adenosine2) \
                              , pd.DataFrame(itSalineGroup0) , pd.DataFrame(itSalineGroup1), pd.DataFrame(itSalineGroup2) \
