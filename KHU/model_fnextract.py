...
for SE in range(N):
    for se in range(len(signalss[SE])):
        ...
        if (np.sum(np.array(painc)) > 0  or np.sum(np.array(nonpainc)) > 0) and  ex_pain: print(SE, se, 'excluded')
...
with open(savename, 'rb') as f:  # Python 3: open(..., 'rb')
    msdata_load = pickle.load(f)
    fn_extracted_data = msdata_load
...
# label 지정
    ...
    #                for ROI in range(signalss[SE][se].shape[1]):
                    ...
                    for u in msbins:
                        ...
                        # feature 2
                        ft2 = np.array(fn_extracted_data[SE][se])
...
for b1 in range(1):
    ...
    def keras_setup(lr=0.01, batchnmr=False, seed=1):
        ...
        concatted = tf.keras.layers.Concatenate()([input10, input2])
        
        input11 = Dense(2**3, kernel_initializer = init, activation='relu')(concatted) # fully conneted layers, relu

        merge_4 = Dense(2, kernel_initializer = init, activation='softmax')(input11) # fully conneted layers, relu
    ...
    ###     project_list
    project_list = []
    project_list.append(['20210618_KHU_1', 100]) # project name, seed
        ...
        for k in range(len(validlist)):
            ...
            if not(stopsw): 
                ...
                if not(os.path.isfile(final_weightsave)) or True:
                    ...
                    for n in range(len(X_tr)):
                        X_tr_1.append(X_tr[n][0])
                        X_tr_2.append(X_tr[n][1])
                        
                    X_tr_1, X_tr_2 = np.array(X_tr_1), np.array(X_tr_2)
                    ...
                    while True:
                        ...
                        # for j in range(10):
                        hist = model.fit([X_tr_1, X_tr_2], Y_tr, batch_size=2**11, epochs=6000, verbose=1, callbacks=callbacks)
