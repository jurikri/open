...
settingID = 'model4.0_0923_feature_reduce_partialSNU_PD_painDimension2' 
...
wantedlist = morphineGroup + KHUsham + KHU_CFA + PDnonpain + PDpain
...
for repeat in range(0, 50):
    ...
    for SE in range(N):
        if repeat % 2 == 0:
            SEix = np.where(np.array(group_nonpain_training)[:,0]==SE)
            selist = list(set(np.array(group_nonpain_training)[SEix,1][0]))
        
        if repeat % 2 == 1:
            SEix = np.where(np.array(group_pain_training)[:,0]==SE)
            selist = list(set(np.array(group_pain_training)[SEix,1][0]))
        
        # if SE in PSLgroup_khu: selist = [0]
...
# RESULT_SAVE_PATH = 'C:\\mass_save\\model3\\model3.1.1_base_in_reverse2_SNU_0826\\' 
#   'C:\\mass_save\\model3\\model3.1.1_base_insample2_SNU_0826\\'
...
l = np.array(mssave_final).shape[0]

mssave_final2 = np.array(mssave_final)[list(range(0,l,2))]
mssave_final2_ax2 = np.array(mssave_final)[list(range(1,l,2))]
...
mssave_ax1 = np.nanmean(mssave_final2, axis=0)
mssave_ax2 = np.nanmean(mssave_final2_ax2, axis=0)


X, Y, Z = [], [], []
for SE in KHU_CFA:
    for n in range(0,4):
        if not(np.isnan(mssave_ax1[SE, n])) and not(np.isnan(mssave_ax2[SE, n])):
            X.append([mssave_ax1[SE, n], mssave_ax2[SE, n]])
            Y.append([-1])
            Z.append([SE, n])
        
    for n in list(range(4,6)) + list(range(8,11)):
        if not(np.isnan(mssave_ax1[SE, n])) and not(np.isnan(mssave_ax2[SE, n])):
            X.append([mssave_ax1[SE, n], mssave_ax2[SE, n]])
            Y.append([1])
            Z.append([SE, n])
...
for SE in morphineGroup:
    for n in range(0,2):
        if not(np.isnan(mssave_ax1[SE, n])):
            X.append([mssave_ax1[SE, n], mssave_ax2[SE, n]])
            Y.append([-1])
            Z.append([SE, n])
...
print(X.shape, len(Y), np.mean(Y, axis=0))
np.mean(X)

 
def keras_setup2(lr=0.01, batchnmr=False, seed=1, add_fn=None):
    #### keras #### keras  #### keras #### keras  ####keras #### keras  #### keras #### keras  #### keras #### keras  #### keras #### keras

    init = initializers.he_uniform(seed=seed) # he initializer를 seed 없이 매번 random하게 사용 -> seed 줌
    input1 = tf.keras.layers.Input(shape=(X[0].shape))
    merge_4 = Dense(1, kernel_initializer = init)(input1) # fully conneted layers, relu

    model = tf.keras.models.Model(inputs=input1, outputs=merge_4) # input output 선언
    model.compile(loss=tf.keras.losses.MeanSquaredError(), optimizer=Adam(learning_rate=lr, decay=1e-8, beta_1=0.9, beta_2=0.999), metrics=['accuracy']) # optimizer

    #### keras #### keras  #### keras #### keras  #### keras #### keras  #### keras #### keras  #### keras #### keras  #### keras #### keras
    return model

model = keras_setup2(lr=lr, seed=0, add_fn=2)
print(model.summary())
hist = model.fit(X, Y, batch_size=2**11, epochs=5000)
...
def model1(x):
    return (-beta1/beta2*x) - (beta0/beta2)


nonpain = np.where(Y==-1)[0]
pain = np.where(Y==1)[0]

class0_0 = X[nonpain][:,0]
class0_1 = X[nonpain][:,1]

class1_0 = X[pain][:,0]
class1_1 = X[pain][:,1]

plt.figure()
plt.scatter(class0_0, class0_1)
plt.scatter(class1_0, class1_1)
axis = np.arange(-5,10,0.5)
plt.plot(axis, model1(axis))
plt.xlim([-1,2])
plt.ylim([-1,2])
...
for i in range(len(Z)):
    mssave[Z[i][0], Z[i][1]] = pred[i]


accuracy, roc_auc = msROC(pred[nonpain], pred[pain]);  print(accuracy, roc_auc)
#%%
plt.figure()
plt.plot(np.nanmean(mssave[morphineGroup,2:], axis=0), c='r')
plt.plot(np.nanmean(mssave[KHUsham,2:], axis=0), c='b')
...
accuracy, roc_auc = msROC(class1, class0); print(accuracy, roc_auc)
...
#%% KHU_CFA
plt.figure()
plt.plot(np.nanmean(mssave[KHU_CFA,:], axis=0), c='r')

plt.figure()
msplot = mssave[KHU_CFA,:]
msplot_mean = np.nanmean(msplot, axis=0)
e = scipy.stats.sem(msplot, axis=0, nan_policy='omit')
plt.errorbar(range(len(msplot_mean)), msplot_mean, e, linestyle='None', marker='o', c='r')

class0 = mssave[KHU_CFA,2:4].flatten()
class1 = list(mssave[KHU_CFA,4:6].flatten()) + list(mssave[KHU_CFA,9:].flatten())
accuracy, roc_auc = msROC(class0, class1); print(accuracy, roc_auc)
...
plt.figure()
msplot = mssave2[KHU_CFA,:]
msplot_mean = np.nanmean(msplot, axis=0)
e = scipy.stats.sem(msplot, axis=0, nan_policy='omit')
plt.errorbar(range(len(msplot_mean)), msplot_mean, e, linestyle='None', marker='o', c='r')

class0 = mssave2[KHU_CFA,0]
class1 = list(mssave2[KHU_CFA,1]) + list(mssave2[KHU_CFA,3])
accuracy, roc_auc = msROC(class0, class1); print(accuracy, roc_auc)


Aprism = mssave2[KHU_CFA,:] - np.min(mssave2[KHU_CFA,:])
