...
settingID = 'model3.2_outsample_0827' 
...
wantedlist = PSLgroup_khu + morphineGroup + KHUsham + highGroup3
outsamplelist = [] # PDpain # PDpain # 여기에 outsample 넣어

# GBVX = [164, 166, 167, 172, 174, 177, 179, 181]
nonlabel_group = morphineGroup + KHUsham  # morphine test 용
...
for repeat in range(0, 2):
    ...
    for SE in range(N):
        ...
        for stanse in selist:
            if [SE, stanse] in group_nonpain_training:
                    ...
                    for se in range(len(target_sig[SE])):
                        if se != stanse:
                            if len(target_sig2[SE][se]) > 0:
                                if np.isnan(np.mean(target_sig2[SE][se])): import sys; sys.exit()   
                                label = None
