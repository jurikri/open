...
total_list = list(set(list(np.array(group_pain_training)[:,0]) + list(np.array(group_nonpain_training)[:,0]) \
                  + list(np.array(group_drug_training)[:,0]) + list(np.array(group_outsamples)[:,0])))
...
settingID = 'model5_20220218_GBVX'
...
wantedlist = pslGroup + shamGroup + gabapentinGroup
...
for lv in range(10):
    ...
    if not(os.path.isfile(savepath)):
        ...
        if False:
            ...
            vis.ms_report_snu_chronic(mssave2[:,:,1])
...
vis.ms_report_snu_chronic(mssave[:,:,1])
