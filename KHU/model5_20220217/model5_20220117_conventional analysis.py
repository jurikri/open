...
for SE in range(N):
    for se in range(len(signalss_raw[SE])):
        t4[SE,se] = np.nanmean(signalss_df[SE][se])

nonpain, pain = [], []
for SE in range(N):
    for se in range(len(signalss_raw[SE])):
        if [SE, se] in group_nonpain_training: nonpain.append(t4[SE,se]) 
        elif [SE, se] in group_pain_training: pain.append(t4[SE,se])

msFunction.msROC(nonpain, pain, figsw=True)

msplot = [nonpain, pain]
fig = plt.figure(1, figsize=(4, 6))
ax = fig.add_subplot(111)
bp = ax.boxplot(msplot, whis=[0, 100], widths=(0.7,0.7))
...
settingID = 'model5_20220218_conventional'
...
for feature_n in range(X.shape[1]):  
    ...
    nonpain = X[:, feature_n][nonpain_ix]
    pain = X[:, feature_n][pain_ix]
    accuracy, roc_auc, fig, msdict = msFunction.msROC(nonpain, pain, figsw=True)
    msdict_save.append(msdict)
    roc_save.append(roc_auc)
    


for feature_n in range(7): # range(X.shape[1]): 
    nonpain_ix = np.where(Y[:,0]==1)[0]
    pain_ix = np.where(Y[:,1]==1)[0]
    nonpain = X[:, feature_n][nonpain_ix]
    pain = X[:, feature_n][pain_ix]
    nonpain = msFunction.nanex(nonpain)
    pain = msFunction.nanex(pain)
    
    msplot = [nonpain, pain]
     
    fig = plt.figure(1, figsize=(5, 20))
    plotn = 710 + feature_n + 1
    ax = fig.add_subplot(plotn)
    labels = ['nonpain', 'pain']
    bp = ax.boxplot(msplot, whis=[0, 100], widths=(0.7,0.7), labels=labels)
    plt.ylim((-0.1,1))
    

    
    
    filepath = 'C:\\SynologyDrive\\worik in progress\\20220110 - 2p_PSL\\figsave\\'
    plt.savefig(filepath + 'fig_' + str(feature_n))


import twophoton_pain_visualization as vis
vis.ROC_merge(msdict_save[:6])
...
for lv in range(1,5):
    ...
    if not(os.path.isfile(savepath)):
        ...
        if False:
            ...
            for SE in range(N):
                for se in range(len(signalss_raw[SE])):
                    if [SE, se] in group_nonpain_training: nonpain.append(mssave2[:,:,1][SE,se]) 
                    elif [SE, se] in group_pain_training: pain.append(mssave2[:,:,1][SE,se])
            msFunction.msROC(nonpain, pain, figsw=True)    
...
for SE in range(N):
    for se in range(len(signalss_raw[SE])):
        if [SE, se] in group_nonpain_training: nonpain.append(mssave[:,:,1][SE,se]) 
        elif [SE, se] in group_pain_training: pain.append(mssave[:,:,1][SE,se])
msFunction.msROC(nonpain, pain, figsw=True)
