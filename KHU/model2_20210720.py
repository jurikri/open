...
#%%     project_list
project_list = []
project_list.append(['20210720_model_1', 100]) # project name, seed
project_list.append(['20210720_model_2', 200])
project_list.append(['20210720_model_3', 300])
            ...
            if os.path.isfile(final_weightsave):
                ...
                for valSE in vlist:
                    for valse in range(0, len(signalss[valSE])):
                        if np.isnan(mssave[valSE, valse]):
...
nonpain = mssave[PSLgroup_khu,0]
pain = mssave[PSLgroup_khu,1:3].flatten()
