...
for SE in range(N):
    if not SE in [179, 181]: # ROI 매칭안되므로 임시 제거
        for se in range(MAXSE):
            ...
            # painc.append(SE in morphineGroup and se in [10,11,12])
            # nonpainc.append(SE in KHUsham and se in range(10,13))
             
            # khu cfa
            nonpainc.append(SE in KHU_CFA and se in [0,1,2,3])
            painc.append(SE in KHU_CFA and se in [4,5,8,9])
            # nonpainc.append(SE in KHU_CFA and se in [6,7]) # lidocaine
...
settingID = 'model3.2_insample_as_nonpain_nonesnu_0831_2' 
...
for repeat in range(0, 10):
    ...
    for SE in range(N):
        ...
        if SE in [247,248,250,251] + [257, 258, 259, 262]: selist = [0,1,3,4]
        for stanse in selist:
            if [SE, stanse] in group_nonpain_training or SE in nonlabel_group:
...
#%% KHU_CFA
plt.figure()
plt.plot(np.nanmean(mssave[KHU_CFA,2:], axis=0), c='r')

msplot = mssave[KHU_CFA,2:]
