...
wantedlist = pslGroup + shamGroup + oxaliGroup + glucoseGroup
...
for repeat in [0]: #range(0, 50):
    ...
    dice_label = list(group_nonpain_training)
    
    for SE in forlist:
        selist = list(range(len(target_sig[SE])))
        passw = False
        # random.shuffle(selist)
        for stanse in [0]: #selist:
                ...
                if not(np.isnan(target_sig2[SE][stanse][0])):
                    ...
                    for se in range(len(target_sig[SE])):
                        ...
                        if not(np.isnan(target_sig2[SE][se][0])):
                            ...
                            # f6
                            msmatrix = signalss[SE][se]
                            
                            tmp1 = np.mean(msmatrix[vix, :], axis=0)/np.mean(msmatrix[vix, :])
                            tmp2 = np.mean(msmatrix[vix2, :], axis=0)/np.mean(msmatrix[vix2, :])
                            
                            f6 = np.mean(np.abs(tmp1 - tmp2) > THR)
                            if np.isnan(f6): f6 = 0
                            ...
                            sig = target_sig[SE][se][vix,:]
                            exp = np.mean(sig, axis=0) / np.mean(sig)
                            f7 = np.mean(exp - stand1 > 0.4)
    
                            # binstmp = 100
                            # msbins = np.arange(0, len(msmatrix)-binstmp, binstmp)
                            
                            # binsave = []
                            # for bb in msbins:
                            #   tmp = msmatrix[bb:bb+binstmp,:]
                            #   if len(tmp) == binstmp:
                            #     binsave.append(np.mean(tmp, axis=0))
                            # binsave = np.array(binsave)
                            # f6 = np.mean(np.std(binsave, axis=0) / np.mean(binsave, axis=0))
                            ...
                            if not(label is None) and se != stanse:
                                ...
                                X.append([f0, f1, f2, f3, f4, f5, f6, f7])
                                Y.append(label)
                                Z.append([SE, se])
    ...
    class_mu_sigma = msFunction.msarray([2,fn]) # classnum, fnnum
    for fnum in range(fn):
        plt.figure(); plt.title('fn_'+ str(fnum))
        samples = X[ix0, fnum]
    ...
    print(1 - np.mean(np.abs(estimate - Y[:,1])))
...
for se in [1,2]:
    class0, class1 = [], []
    class0 += list(mssave[shamGroup,se].flatten())
    class1 += list(mssave[pslGroup,se].flatten())
    accuracy, roc_auc = msROC(class0, class1)
    print('SNU PSL', str(se), accuracy, roc_auc)

plt.figure()
plt.title('SNU oxaliplatin')
plt.plot(np.nanmean(mssave[pslGroup,:], axis=0))
plt.plot(np.nanmean(mssave[shamGroup,:], axis=0))
for se in [1,2]:
    class0, class1 = [], []
    class0 += list(mssave[glucoseGroup,se].flatten())
    class1 += list(mssave[oxaliGroup,se].flatten())
    accuracy, roc_auc = msROC(class0, class1)
    print('SNU oxaliplatin', str(se), accuracy, roc_auc)
