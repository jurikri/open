...
for SE in range(N):
    if not SE in [179, 181]: # ROI 매칭안되므로 임시 제거
        for se in range(MAXSE):
            ...
            nonpainc.append(SE in glucoseGroup and se in [0,1,2,3])
...
mssave_final = []
settingID = 'model3_insample_oxsaliplatin_inset_0719'
...
# wantedlist = salineGroup + highGroup + midleGroup + ketoGroup + highGroup2 + CFAgroup + capsaicinGroup + \
#     shamGroup + pslGroup + ipsaline_pslGroup + ipclonidineGroup + glucoseGroup + oxaliGroup + PSLgroup_khu + morphineGroup + PDpain + PDnonpain
    
wantedlist = shamGroup + pslGroup + glucoseGroup + oxaliGroup
    
extraset = morphineGroup + oxaliGroup # unlabel 일때, extraset group이면 test는 진행함
            ...
            if not(np.isnan(target_sig2[SE][stanse][0])):
                ...
                for se in range(len(target_sig[SE])):
                    ...
                    if not(np.isnan(target_sig2[SE][se][0])):
                        ...
                        elif label is None and se != stanse and SE in extraset:
...
mssave[oxaliGroup,:5]
np.nanmean(mssave[oxaliGroup,:5], axis=0)
...
plt.figure()
plt.plot(np.nanmean(mssave[highGroup + highGroup2,:], axis=0))
plt.plot(np.nanmean(mssave[midleGroup,:], axis=0))
plt.plot(np.nanmean(mssave[ketoGroup,:], axis=0))

plt.figure()
plt.plot(np.nanmean(mssave[CFAgroup,:], axis=0))
plt.plot(np.nanmean(mssave[capsaicinGroup,:], axis=0))
