...
def msreport_SNU_oxaliplatin(msresult):
    gsave = msFunction.msarray([4*2]) # d0,3,7,10
    # 0 - pain   4 - nonpain
    gsave[0] += list(np.nanmean(msresult[oxaliGroup ,:][:,[0,1]], axis=1))
    gsave[3] += list(np.nanmean(msresult[[188,200],:][:,[4,5]], axis=1))
    gsave[3] += list(np.nanmean(msresult[[192, 194, 196, 202, 220],:][:,[6, 7]], axis=1))
    gsave[1] += list(np.nanmean(msresult[oxaliGroup,:][:,[2,3]], axis=1))
    gsave[2] += list(np.nanmean(msresult[[192, 194, 196, 202, 220, 198],:][:,[4,5]], axis=1))
    
    gsave[4] += list(np.nanmean(msresult[glucoseGroup,:][:,[0,1]], axis=1))
    gsave[5] += list(np.nanmean(msresult[glucoseGroup,:][:,[2,3]], axis=1))
    gsave[6] += list(np.nanmean(msresult[[206, 208, 210, 212, 214, 222],:][:,[4,5]], axis=1))
    gsave[7] += list(np.nanmean(msresult[[206, 208, 210, 212, 214, 222],:][:,[6,7]], axis=1))
    gsave[7] += list(np.nanmean(msresult[[204],:][:,[4,5]], axis=1))
    
    gsave2 = msFunction.msarray([8])
    sizesave = []
    for i in range(len(gsave)):
        gsave2[i] = msFunction.nanex(gsave[i])
        sizesave.append(len(gsave2[i]))
        print(np.mean(gsave2[i]))
    
    mssave_p2 = np.zeros((np.max(sizesave), 8)) * np.nan
    for i in range(len(gsave2)):
        mssave_p2[:len(gsave2[i]), i] = gsave2[i]
...
if False:
    ...
    Aprism = msreport_SNU_pslGBVX(mssave[:,:,1])
    ...
    # oxaliplatin
    filepath = 'C:\\SynologyDrive\\2p_data\\'
    RESULT_SAVE_PATH = filepath + 'model5_20220208_oxaliplatin'
    savepath = RESULT_SAVE_PATH + '\\repeat_save.pickle'
    with open(savepath, 'rb') as f:  # Python 3: open(..., 'rb')
        repeat_save = pickle.load(f)
        mssave = np.nanmean(np.array(repeat_save), axis=0)
    
    Aprism = msreport_SNU_oxaliplatin(mssave[:,:,1])
    
    pain = Aprism[:,1:3]
    nonpain = list(Aprism[:,5:8].flatten())
    _,_,_, msdict1 = msFunction.msROC(nonpain, pain, figsw=True)

    # revision _ SNU _formalin late

    filepath = 'C:\\SynologyDrive\\2p_data\\'
    RESULT_SAVE_PATH = filepath + 'AIBRNN_20220307_early_late'
    savepath = RESULT_SAVE_PATH + '\\repeat_save.pickle'
    with open(savepath, 'rb') as f:  # Python 3: open(..., 'rb')
        repeat_save = pickle.load(f)
        mssave = np.nanmean(np.array(repeat_save), axis=0)
    ...
    plt.figure()
    msplot = mssave[highGroup + highGroup2 + midleGroup,:]
    msplot_mean = np.nanmean(msplot, axis=0)
    e = scipy.stats.sem(msplot, axis=0, nan_policy='omit')
    plt.errorbar(range(len(msplot_mean)), msplot_mean, e, linestyle='None', marker='o', c='r')
