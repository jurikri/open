...
def keras_setup(lr=1e-3, xin=None, add_fn=None, seed=0, l2=0, layer_1=10, \
                n_hidden=10, batchnmr=False, dropout_rate1=0, dropout_rate2=0):
...
#%%
savepath = 'C:\\SynologyDrive\\2p_data\\'+ 'shallow_featrues.pickle'
with open(savepath, 'rb') as f:  # Python 3: open(..., 'rb')
    msdict = pickle.load(f)
    X_shallow = msdict['X']
    Y_shallow = msdict['Y']
    Z_shallow = np.array(msdict['Z'])
...
l2_rate = 0.1
#%% overfit
model = keras_setup(lr=1e-3, xin=X_total1[0], add_fn=X_total2[1].shape[0], seed=0, l2=l2_rate, layer_1=layer_1, \
                    n_hidden=n_hidden, batchnmr=True, dropout_rate1=dropout_rate1, dropout_rate2=dropout_rate2)
print(model.summary())
    
#%%
hist = model.fit([X_total1, X_total2], Y_total, batch_size=2**11, epochs=epochs, verbose=1)

xhat = model.predict([X1, X2])
mssave_total = msFunction.msarray([N,MAXSE])
for i in range(len(Z)):
    teSE = Z[i][0]
    tese = Z[i][1]
    mssave_total[teSE][tese].append(xhat[i][1])

mssave2 = np.zeros((N,MAXSE)) * np.nan
for row in range(N):
    for col in range(MAXSE):
        tmp = np.nanmean(mssave_total[row][col])
        if not(np.isnan(np.mean(tmp))): mssave2[row, col] = tmp
        
plt.figure()
msplot = mssave2[highGroup + midleGroup + highGroup2,:4]
...
if True:
    ...
    for SE in wantedlist:
        cvlist_tmp = []
        for se in range(MAXSE):
            i = SEse_find(Z=Z_total, SE=SE, se=se)
...
for repeat in range(10): #, 100):
    ### outsample test
    print('repeat', repeat, 'data num', len(Y_total), 'Y2 dis', np.mean(Y_total, axis=0))
    meansave = msFunction.msarray([N,MAXSE])
    
    totallist = list(range(len(Y_total)))
    for cv in range(0, len(cvlist)):
        telist = cvlist[cv, 0]
        if len(telist) > 0:
            trlist = list(set(totallist)-set(telist))
            # print(len(totallist), len(trlist), len(telist))
            X_tr1 = X_total1[trlist]; X_te1 = X_total1[telist]
            X_tr2 = X_total2[trlist]; X_te2 = X_total2[telist]
            Y_tr = Y_total[trlist]; Y_te = Y_total[telist]
            Z_tr = Z_total[trlist]; Z_te = Z_total[telist]
            ...
            if not(os.path.isfile(final_weightsave)) or overwrite:
                ...
                verbose = 0
                if cv == 0: verbose = 1
                hist = model.fit([X_tr1, X_tr2], Y_tr, batch_size=2**11, epochs=epochs, verbose=verbose)
                model.save_weights(final_weightsave)
                
            # test
            model.load_weights(final_weightsave)
            yhat = model.predict([X_te1, X_te2])
            
            for n in range(len(yhat)):
                teSE = Z_te[n][0]; tese = Z_te[n][1]
                meansave[teSE][tese].append(yhat[n])

            # outsample
            outlist = np.where(np.sum(Y, axis=1)==0)[0]
            yhat_out = model.predict([X1[outlist], X2[outlist]])
            for out_SE in wantedlist:
                for n in np.where(Z[outlist][:,0]==out_SE)[0]:
                    teSE = Z[outlist][n][0]; tese = Z[outlist][n][1]
                    meansave[teSE][tese].append(yhat_out[n])
                    
    mssave2 = np.zeros((N,MAXSE)) * np.nan
    for row in range(N):
        for col in range(MAXSE):
            tmp = np.nanmean(meansave[row][col], axis=0)
...
#%%
plt.figure()
msplot = mssave[highGroup + midleGroup + highGroup2,:4]
msplot_mean = np.nanmean(msplot, axis=0)
e = scipy.stats.sem(msplot, axis=0, nan_policy='omit')
plt.errorbar(range(len(msplot_mean)), msplot_mean, e, linestyle='None', marker='o', c='b')

msplot = mssave[ketoGroup,:4]
msplot_mean = np.nanmean(msplot, axis=0)
e = scipy.stats.sem(msplot, axis=0, nan_policy='omit')
plt.errorbar(range(len(msplot_mean)), msplot_mean, e, linestyle='None', marker='o', c='g')
