...
project_list = []
 # proejct name, seed
project_list.append(['1128_binfix3', 3, None])
...
# In[]    
    ...
    def ms_sampling():
        ...
        print('부족한 pain samples #', diff, 'GAN data로 보충합니다.')
              
        GAN_SAVE_PATH = savepath + 'GAN\\' + '1128_GAN1' + '\\GANdata\\'
        loadname = GAN_SAVE_PATH + 'GANdata_sett_for_0.csv'

        GAN_Data = pd.read_csv(loadname, header=None)
        GAN_Data = np.array(GAN_Data)
        
        SE = 1; se = 1; msclass = 1
        print('GAN data 처리중.. 시간걸림')
        for dataNum in range(GAN_Data.shape[0]):
            mannual_signal = np.array(GAN_Data[dataNum,:])
            mannual_signal = np.reshape(mannual_signal, (mannual_signal.shape[0], 1))
            X, Y, Z, _ = dataGeneration(SE, se, label=msclass, Mannual=True, mannual_signal=mannual_signal)
            X_tmp += X; Y_tmp += Y; Z_tmp += Z #; T_tmp += t4_save 
            
            if len(X_tmp) > diff:
                print('GAN data #', len(X_tmp))
                break
            
        datasetX[msclass] = np.concatenate((datasetX[msclass], np.array(X_tmp)), axis=0)
        datasetY[msclass] = np.concatenate((datasetY[msclass], np.array(Y_tmp)), axis=0)
        datasetZ[msclass] = np.concatenate((datasetZ[msclass], np.array(Z_tmp)), axis=0)
