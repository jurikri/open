        ...
        if not(os.path.exists(savepath) and skipsw): 
            if SE in snuformat1:
                ...
                for se in range(k):
                    ...
                    array0[se] = array0[se] / (2**8) # khuformat specific process
                    print(str(SE) + ' max ' + str(np.max(np.max(array0[se]))) + \
                          ' min ' +  str(np.min(np.min(array0[se]))))
...
signalss, behavss, roi_del_ix_save = mssignal_save(list1=runlist, gfiltersw=True, skipsw =True)
